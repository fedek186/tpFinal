...
//cargamos los articulos y tenemos ya todos los generos por serie
    for (let i = 0; i < 5; i++) {
      let generoSeries= `<article><a href="detail_genre_series.html?id=${info[i].id}&tipo=series&nombre=${info[i].name}">${info[i].name}</a></article>`
...
.then(function(data) {
  ...
  for (let i = 0; i < 5; i++) {
      let generosPeliculas= `<article><a href="detail_genre_movies.html?id=${info[i].id}&tipo=peliculas&nombre=${info[i].name}">${info[i].name}</a></article>`
