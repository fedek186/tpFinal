  ...
  .then(function (data) {
    ...
    for (let i = 0; i < data.genres.length; i++) {
     ...
     if (localStorage.getItem("idSeries") == null) { //Existe algo guardado en el local? No
       let arrayIdSeries = []; //Creamos array Id
       localStorage.setItem("idSeries", JSON.stringify(arrayIdSeries)); //Guardamos el array en el local
     } else { //Existe algo guardado en el local? Si
       let arraySeries = JSON.parse(localStorage.getItem("idSeries")); //Traemos el array
       if (arraySeries.indexOf(id) != -1) { //Esta en el array? Si 
    ...
    //FAVORITOS parte II
     corazon.addEventListener("click", function (e) {
       ...
       if (array.indexOf(id) != -1) { //Esta en el array? SI
         ...
         localStorage.setItem("idSeries", JSON.stringify(array)); //Guardamos el array sin el
         ...
         localStorage.setItem("idSeries", JSON.stringify(array)); //Lo guardamos
       }
     });
  })
